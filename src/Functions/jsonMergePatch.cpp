    ...
    private:
            ...
            size_t input_rows_count) const
        {
            ...
            for (size_t row = 0; row < input_rows_count; ++row)
            {
                ...
                Object merged = JSONMergePatchHelpers::extractObject(arguments[0], row);

                /// Merge each subsequent argument
                for (size_t arg_idx = 1; arg_idx < arguments.size(); ++arg_idx)
                {
                    Object current = JSONMergePatchHelpers::extractObject(arguments[arg_idx], row);
                    JSONMergePatchHelpers::mergeObjects(merged, current);
                }
