...
#undef GENERATE_INTERVAL_CASE
    ...
    /// Direct converters for Map/Tuple to Object that bypass string serialization
    struct ConvertMapToObject
    {
        static ColumnPtr execute(
            ColumnsWithTypeAndName & arguments,
            const DataTypePtr & result_type,
            size_t input_rows_count)
        {
            const auto & source_column = arguments[0].column;
            const auto & map_column = assert_cast<const ColumnMap &>(*source_column);
            const auto & nested_array = map_column.getNestedColumn();
            const auto & nested_tuple = map_column.getNestedData();
            const auto & keys_column = assert_cast<const ColumnString &>(nested_tuple.getColumn(0));
            const auto & values_column = nested_tuple.getColumn(1);
            ...
            for (size_t row = 0; row < input_rows_count; ++row)
            {
                ...
                {
                    StringRef key_ref = keys_column.getDataAt(i);
                    String key(key_ref.data, key_ref.size);
    ...
    struct ConvertNamedTupleToObject
    {
        static ColumnPtr execute(
            ColumnsWithTypeAndName & arguments,
            const DataTypePtr & result_type,
            size_t input_rows_count)
        {
            ...
            const auto & tuple_column = assert_cast<const ColumnTuple &>(*source_column);
            const auto & tuple_type = assert_cast<const DataTypeTuple &>(*source_type);

            const auto & field_names = tuple_type.getElementNames();
            size_t num_fields = tuple_column.tupleSize();
            ...
            for (size_t row = 0; row < input_rows_count; ++row)
            {
                ...
                {
                    const String & field_name = field_names[field_idx];
                    const IColumn & field_column = tuple_column.getColumn(field_idx);

                    Field value;
                    field_column.get(row, value);
                    obj[field_name] = std::move(value);
                }
    ...
    WrapperType createObjectWrapper(const DataTypePtr & from_type, const DataTypeObject * to_object, bool requested_result_is_nullable) const
    {
        ...
        /// Direct conversion from Map to JSON (bypass string serialization)
        if (checkAndGetDataType<DataTypeMap>(from_type.get()))
        {
            return [](ColumnsWithTypeAndName & arguments, const DataTypePtr & result_type, const ColumnNullable *, size_t input_rows_count)
            {
                return ConvertMapToObject::execute(arguments, result_type, input_rows_count);
            };
        }

        /// Direct conversion from named Tuple to JSON (bypass string serialization)
        if (const auto * tuple_type = checkAndGetDataType<DataTypeTuple>(from_type.get()))
        {
            if (tuple_type->hasExplicitNames())
            {
                return [](ColumnsWithTypeAndName & arguments, const DataTypePtr & result_type, const ColumnNullable *, size_t input_rows_count)
                {
                    return ConvertNamedTupleToObject::execute(arguments, result_type, input_rows_count);
                };
            }
        }

        /// Cast Object/JSON to JSON type through serializing into JSON string and parsing back into JSON column.
