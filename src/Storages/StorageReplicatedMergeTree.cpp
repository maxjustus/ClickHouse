...
void StorageReplicatedMergeTree::mergeSelectingTask()
{
    ...
    auto try_assign_merge = [&]() -> AttemptStatus
    {
        ...
        if (can_assign_merge)
        {
            /// When partition-batch selection is enabled, skip the full-table discovery scan and try a
            /// bounded round-robin batch of partitions directly. Forced whole-partition merges rely on
            /// statistics gathered from all partitions, so keep full discovery when they are enabled.
            ...
            {
                partitions_to_merge_in = pickMergeSelectingPartitionsBatch(partitions_batch_size);
            }
            else
            {
