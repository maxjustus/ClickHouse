...
/// filtering afterwards. This is exactly equivalent to filterByPartitions(collectInitial(data), hint):
/// the parts index is ordered by (state, kind, partition_id, ...) and DataPartStateAndPartitionID derives
/// the kind from the partition id, so a per-partition lookup returns the same parts (patch partitions
/// included) that the filter would keep. The collected parts are sorted to match collectInitial's order.
